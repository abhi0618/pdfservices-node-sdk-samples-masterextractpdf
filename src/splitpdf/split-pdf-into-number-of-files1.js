...
try {
    ...
    const splitPDFOperation = PDFServicesSdk.SplitPDF.Operation.createNew(),
        input = PDFServicesSdk.FileRef.createFromLocalFile(
            'resources/MultipleFilesOperationTesting/combinepdf2pdfs.pdf',
    ...
    splitPDFOperation.execute(executionContext)
        .then(result => {
            ...
            for(let i = 0; i < result.length; i++){
                saveFilesPromises.push(result[i].saveAsFile(`output/splitPDFIntoNumberOfFilesOutput2pdfcombined_${i}.pdf`));
            }
